...
function setClothes() {
    ...
    set.forEach(function(element, index, array) {
        if(!inArray(element, loadedClothes)) {
            ...
            div.style.backgroundImage = "url('https://avatar-retro.com/habbo-imaging/avatarimage?cache=28072017&figure="+Avatargenerator.currentSet+"-"+element+"-"+color+"')";
                ...
                for (var nKey in clothesObjects) {
                    ...
                    if(typeof nCObject.style != 'undefined') {
                        if(typeof nCObject.getAttribute('remove') == 'string' && nCObject.getAttribute('remove') == '0') {
                            nCObject.style.backgroundImage = "url('https://avatar-retro.com/habbo-imaging/avatarimage?cache=28072017&figure="+Avatargenerator.currentSet+"-"+nCObject.getAttribute('element')+"-"+e[0]+"')";
                        }
                    }
                }
                return drawAvatar();
...
function drawAvatar() {
    document.getElementById("avatar").style.backgroundImage = "url('https://avatar-retro.com/habbo-imaging/avatarimage?cache=28072017&figure="+Avatargenerator.generateCode()+"&size=l&direction="+direction+"')";
    document.getElementById('code').innerHTML = '<input type="input" id="changelook" value="https://avatar-retro.com/habbo-imaging/avatarimage?cache=28072017&figure='+Avatargenerator.generateCode()+'">';
}
